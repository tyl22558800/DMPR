...
def boundary_check(centralied_marks):
    ...
    for mark in centralied_marks:
        if mark[0] < -124 or mark[0] > 124 or mark[1] < -380 or mark[1] > 380:
            return False
    return True
...
def overlap_check(centralied_marks):
    ...
    for i in range(len(centralied_marks) - 2):
        i_x = centralied_marks[i, 0]
        i_y = centralied_marks[i, 1]
        for j in range(i + 1, len(centralied_marks)-1):
            j_x = centralied_marks[j, 0]
            j_y = centralied_marks[j, 1]
            if abs(j_x - i_x) < 256 / 16 and abs(j_y - i_y) < 768 / 16:
                centralied_marks = np.delete(centralied_marks, [j], axis=0)
...
def generalize_marks(centralied_marks):
    ...
    generalized_marks = []
    centralied_marks_1 = np.unique(centralied_marks, axis=0)
    for mark in centralied_marks_1:
        xval = (mark[0]+128) / 256# xval = (mark[0] + 300) / 600
        yval = (mark[1]+384) / 768# yval = (mark[1] + 300) / 600
...
def generate_dataset(args):
    ...
    for label_file in os.listdir(args.label_directory):
        ...
        centralied_marks = np.array(label['marks_1'])
        if len(centralied_marks.shape) < 2:
            centralied_marks = np.expand_dims(centralied_marks, axis=0)
        centralied_marks[:, 0] -= 128
        centralied_marks[:, 2] -= 128
        centralied_marks[:, 1] -= 384
        centralied_marks[:, 3] -= 384
        centralied_marks = centralied_marks.astype(float)
