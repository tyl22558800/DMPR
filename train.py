...
def generate_objective(marking_points_batch, device):
    ...
    objective = torch.zeros(batch_size, config.NUM_FEATURE_MAP_CHANNEL,
                            config.FEATURE_MAP_SIZE_y, config.FEATURE_MAP_SIZE_x,
                            device=device)
    ...
    for batch_idx, marking_points in enumerate(marking_points_batch):
        for marking_point in marking_points:
            col = math.floor(marking_point.x * config.FEATURE_MAP_SIZE_x)
            row = math.floor(marking_point.y * config.FEATURE_MAP_SIZE_y)
            # Confidence Regression
            objective[batch_idx, 0, row, col] = 1.
            # Makring Point Shape Regression
            objective[batch_idx, 1, row, col] = marking_point.shape
            # Offset Regression
            objective[batch_idx, 2, row, col] = marking_point.x*8 - col
            objective[batch_idx, 3, row, col] = marking_point.y*24 - row
...
def train_detector(args):
    ...
    dp_detector = DirectionalPointDetector(
        3, args.depth_factor, config.NUM_FEATURE_MAP_CHANNEL, descriptor_dim=128).to(device)
    ...
    f = open(r'logs\loss_txt\loss.txt', "r+")
    f.truncate()  # 清除txt文件里面的内容
    f.write('\n' + str(j))  # 往txt文件写入x数组数据（按列写入）
    f.write('\n' + str(t_loss))  # 往txt文件写入y数组数据（按列写入）
