...
class EdgePredictor(nn.Module):
    ...
    def __init__(self, input_dim, layers):
        ...
        self.encoder = MLP([input_dim] + layers + [1], drop_out=True)
...
class GCNEncoder(nn.Module):
    def __init__(self, type, output_dim, layers, k, gat_layers, proj_dim):
        super(GCNEncoder, self).__init__()

        self.feat_dim = output_dim

        self.point_encoder = PointEncoder(2, self.feat_dim, layers)
        
        if type == 'GAT':
            self.gnn = AttentionalGNN(self.feat_dim, gat_layers)
        elif type == 'DGCNN':
            self.gnn = DGCNN(self.feat_dim, self.feat_dim, k=k)
        else:
            raise ValueError("Unknown GCNEncoder type {}".format(type))

        self.proj = nn.Conv1d(self.feat_dim, proj_dim, kernel_size=1, bias=True)
